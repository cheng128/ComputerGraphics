...
//	fragColor = vec4(TexCoord.xy, 0, 1);
    if (VertexPixel == 0) {
        ...
        FragColor = texture(ourTexture, TexCoord + TexCoordShift) * vec4(result, 1.0);
    } else {
        // vertex lighting
        FragColor = texture(ourTexture, TexCoord + TexCoordShift) * vec4(vertex_color, 1.0);
    }
	// [TODO] sampleing from texture
	// Hint: texture (跟 lighting 做 modulation)
}
