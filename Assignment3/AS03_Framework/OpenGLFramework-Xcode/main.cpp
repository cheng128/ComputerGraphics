...
bool spotLightEff = true;

// control maginification texture filtering mode
...
void setFiltering(int eye){
    if (eye) {
        switch (models[cur_idx].cur_eye_offset_idx) {
            case 1:
                glUniform2f(TexCoordShiftLoc, 0.0, 0.0);
                break;
            case 2:
                glUniform2f(TexCoordShiftLoc, 0.0, -0.25);
                break;
            case 3:
                glUniform2f(TexCoordShiftLoc, 0.0, -0.5);
                break;
            case 4:
                glUniform2f(TexCoordShiftLoc, 0.0, -0.75);
                break;
            case 5:
                glUniform2f(TexCoordShiftLoc, 0.5, -1.0);
                break;
            case 6:
                glUniform2f(TexCoordShiftLoc, 0.5, -0.25);
                break;
            case 7:
                glUniform2f(TexCoordShiftLoc, 0.5, -0.5);
                break;
        }
        if (cur_idx == 4 || cur_idx == 6) {
...
void RenderScene(int per_vertex_or_per_pixel) {
    ...
    glUniform1i(iLocVertexPixel, per_vertex_or_per_pixel);

	for (int i = 0; i < models[cur_idx].shapes.size(); i++) 
    {   passModelUniform(i);
		glBindVertexArray(models[cur_idx].shapes[i].vao);
        
		// [TODO] Bind texture and modify texture filtering & wrapping mode(glTexParameteri)
...
void print_info(){
    cout << "current eye ID: " << models[cur_idx].cur_eye_offset_idx << endl;
    if (Light_mode == 0) {
	...
	if (action == GLFW_PRESS) {
		switch (key)
		{
            ...
            case GLFW_KEY_Z:
                ...
                break;
            case GLFW_KEY_X:
                cur_idx = (cur_idx - 1 + (int)model_list.size()) % model_list.size();
                break;
            case GLFW_KEY_O:
                if (cur_proj_mode == Perspective)
                {
            ...
            case GLFW_KEY_RIGHT:
                if (models[cur_idx].hasEye) {
                    if (models[cur_idx].cur_eye_offset_idx < models[cur_idx].max_eye_offset) {
            ...
            case GLFW_KEY_LEFT:
                if (models[cur_idx].hasEye) {
                    if (models[cur_idx].cur_eye_offset_idx > 1) {
                        models[cur_idx].cur_eye_offset_idx -= 1;
                    } else {
                        models[cur_idx].cur_eye_offset_idx = 7;
                    }
                }
                break;
            default:
...
GLuint LoadTextureImage(string image_path)
{
	...
	int require_channel = 4;        // 4 for RGBA
...
void setUniformVariables()
{
    ...
    iLocTexture = glGetUniformLocation(program, "ourTexture");
}
...
int main(int argc, char **argv)
{
    ...
    // create window
	GLFWwindow* window = glfwCreateWindow(WINDOW_WIDTH, WINDOW_HEIGHT, "109062633 HW3", NULL, NULL);
    if (window == NULL)
    {
